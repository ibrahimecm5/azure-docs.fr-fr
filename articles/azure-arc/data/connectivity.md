...
description: Décrit en détail les options de connectivité des services de données avec Azure Arc depuis votre environnement vers Azure.
services: azure-arc
...
Les services de données avec Azure Arc vous permettent de vous connecter à Azure dans deux *modes de connexion* différents : 
...
Notez que, si les services de données avec Azure Arc sont directement connectés à Azure, les utilisateurs peuvent utiliser des [API Azure Resource Manager](/rest/api/resources/), l’interface Azure CLI et le portail Azure pour utiliser les services de données Azure Arc. L’expérience en mode directement connecté est très similaire à l’utilisation d’un autre service Azure avec l’approvisionnement, le désapprovisionnement, la mise à l’échelle, la configuration, etc. dans le Portail Azure.  Si les services de données avec Azure Arc sont indirectement connectés à Azure, le portail Azure est une vue en lecture seule. Vous pouvez afficher l’inventaire des instances managées SQL et des instances Postgres Hyperscale que vous avez déployées, ainsi que les détails les concernant, mais vous ne pouvez pas effectuer d’action sur ceux-ci dans le Portail Azure.  En mode de connexion indirecte, toutes les actions doivent être effectuées localement à l’aide d’Azure Data Studio, de l’interface CLI appropriée ou des outils natifs Kubernetes comme kubectl.
...
|**Disponibilité actuelle**| Disponible |disponible en préversion.|Actuellement non pris en charge.|
...
### <a name="helm-chart-used-to-create-data-controller-in-direct-connected-mode"></a>Graphique Helm utilisé pour créer le contrôleur de données en mode de connexion directe

Le graphique Helm utilisé pour approvisionner les objets de programme d’amorçage et de niveau de cluster d’Azure Arc Data Controller, tels que les définitions de ressources personnalisées, les rôles de cluster et les liaisons de rôle de cluster, est extrait d’Azure Container Registry.
...
Azure Data Studio et Azure CLI se connectent aux API Azure Resource Manager pour envoyer et récupérer des données vers et depuis Azure pour certaines fonctionnalités.

#### <a name="connection-source"></a>Source de connexion

Ordinateur exécutant Azure Data Studio ou Azure CLI qui se connecte à Azure.
...
Un ordinateur exécutant Azure CLI qui charge les journaux ou les métriques de surveillance dans Azure Monitor.
...
> [!NOTE]
> Pour le moment, toutes les connexions HTTPS/443 du navigateur au contrôleur de données pour exécuter la commande `az arcdata dc export` et les tableaux de bord Grafana et Kibana et de l’outil sont chiffrés par SSL à l’aide de certificats auto-signés.  À l’avenir, une fonctionnalité vous permettra de fournir vos propres certificats pour le chiffrement de ces connexions SSL.
