...
description: Découvrez quand utiliser Azure Container Apps et en quoi il diffère d’autres options de conteneur, notamment Azure Container Instances, Azure App Service, Azure Functions et Azure Kubernetes Service.
...
Azure Spring Cloud permet de déployer facilement des applications de microservices Spring Boot sur Azure, et sans aucune modification du code. Le service gère l’infrastructure des applications Spring Cloud, ce qui permet aux développeurs de se concentrer sur leur code. Azure Spring Cloud assure la gestion du cycle de vie en utilisant des outils complets, tels que la supervision et les diagnostics, la gestion des configurations, la découverte de services, l’intégration CI/CD, les déploiements bleus-verts, etc. Si votre équipe ou votre organisation utilise majoritairement Spring, Azure Spring Cloud est une option idéale.
