...
- **Sauvegarde sans agent :** Le serveur de sauvegarde Azure ne nécessite pas l’installation d’un agent sur le serveur vCenter ou ESXi pour sauvegarder la machine virtuelle. Au lieu de cela, fournissez l'adresse IP ou le nom de domaine complet (FQDN), ainsi que les informations d'identification utilisées pour authentifier le serveur VMware auprès du serveur de sauvegarde Azure.
...
:::image type="content" source="media/azure-vmware-solution-backup/deploy-backup-server-azure-vmware-solution-diagram.png" alt-text="Diagramme illustrant un serveur de sauvegarde Azure déployé en tant que machine virtuelle Azure IaaS pour protéger les machines virtuelles d'Azure VMware Solution." border="false":::
...
1. Connectez-vous au [portail Azure](https://portal.azure.com/) et, dans le menu de gauche, sélectionnez **Tous les services**.
...
1. Entrez les valeurs, puis sélectionnez **Créer**.
...
1. Si l’un de vos coffres Recovery Services est déjà ouvert, passez à l’étape suivante. 

   >[!TIP]
   >Si aucun coffre Recovery Services n'est ouvert et que vous vous trouvez sur le portail Azure, dans la liste des ressources, entrez **Recovery Services** > **Coffres Recovery Services**.
...
1. Dans la liste des archivages de Recovery Services, sélectionnez un archivage.
   ...
   :::image type="content" source="../backup/media/backup-azure-microsoft-azure-backup/vault-dashboard.png" alt-text="Capture d'écran du tableau de bord des clés.":::
...
1. Sélectionnez **Sauvegarde** pour ouvrir l’Assistant **Prise en main**.
   ...
   :::image type="content" source="../backup/media/backup-azure-microsoft-azure-backup/getting-started-backup.png" alt-text="Capture d'écran dans laquelle l'option Sauvegarde est sélectionnée sous l'Assistant Prise en main.":::

1. Dans la fenêtre qui s’ouvre :

   1. Dans le menu **Où s’exécute votre charge de travail ?** , sélectionnez **Local**.

      :::image type="content" source="media/azure-vmware-solution-backup/deploy-mabs-on-premises-workload.png" alt-text="Capture d'écran des options Où s'exécute votre charge de travail ? et Que voulez-vous sauvegarder ?":::
   ...
   1. Sélectionnez **Préparer l’infrastructure** pour télécharger et installer le serveur de sauvegarde Azure et les informations d’identification du coffre.
      ...
      :::image type="content" source="media/azure-vmware-solution-backup/deploy-mabs-prepare-infrastructure.png" alt-text="Capture d'écran illustrant l'étape de préparation de l'infrastructure.":::
...
1. Dans la fenêtre **Préparer l’infrastructure** qui s’ouvre :
   ...
   :::image type="content" source="media/azure-vmware-solution-backup/deploy-mabs-prepare-infrastructure-2.png" alt-text="Capture d'écran illustrant les étapes de préparation de l'infrastructure pour un serveur de sauvegarde Azure.":::
...
1. Sur la page de téléchargement, sélectionnez tous les fichiers et sélectionnez **Suivant**.
   ...
   :::image type="content" source="../backup/media/backup-azure-microsoft-azure-backup/downloadcenter.png" alt-text="Capture d'écran montrant les fichiers Sauvegarde Microsoft Azure à télécharger.":::
...
1. Sélectionnez **Extraire** pour démarrer le processus d'extraction.
   ...
   :::image type="content" source="../backup/media/backup-azure-microsoft-azure-backup/extract/03.png" alt-text="Capture d'écran montrant les fichiers Sauvegarde Microsoft Azure prêts à être extraits.":::
...
1. Pour déterminer si les configurations matérielles et logicielles requises pour le serveur de sauvegarde Azure sont respectées, sélectionnez **Vérifier à nouveau**. Si c’est le cas, sélectionnez **Suivant**.
...
1. Le package d’installation du serveur de sauvegarde Azure fait partie d’une offre groupée. Il est fourni avec les fichiers binaires SQL Server appropriés requis. Lorsque vous démarrez une nouvelle installation de serveur de sauvegarde Azure, sélectionnez l’option **Installer une nouvelle instance de SQL Server avec ce programme d’installation**. Sélectionnez ensuite **Vérifier et Installer**.
   ...
   :::image type="content" source="../backup/media/backup-azure-microsoft-azure-backup/sql/01.png" alt-text="Capture d'écran montrant la boîte de dialogue des paramètres SQL dans laquelle l'option Installer une nouvelle instance de SQL Server avec ce programme d'installation est sélectionnée.":::
...
1. Fournissez un emplacement pour l’installation des fichiers du serveur Sauvegarde Microsoft Azure, puis sélectionnez **Suivant**.
   ...
   :::image type="content" source="../backup/media/backup-azure-microsoft-azure-backup/space-screen.png" alt-text="Capture d'écran des paramètres SQL Server.":::
