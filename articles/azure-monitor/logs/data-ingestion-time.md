...
- **Délai d’indexation** : temps nécessaire à l’ingestion d’un enregistrement de journal dans le magasin Big Data Azure Monitor.
...
| Journaux IIS et journaux personnalisés | collecte une fois l’horodateur modifié | Pour les journaux d’activité IIS, cela est influencé par le [calendrier de substitution configuré sur IIS](../agents/data-sources-iis-logs.md). |
| Solution Active Directory Replication | Évaluation tous les cinq jours | L’agent collecte ces journaux d’activité uniquement lorsque l’évaluation est effectuée.|
| Solution d’Évaluation d’Active Directory | évaluation hebdomadaire de votre infrastructure Active Directory | L’agent collecte ces journaux d’activité uniquement lorsque l’évaluation est effectuée.|
...
### <a name="azure-metrics-resource-logs-activity-log"></a>Métriques Azure, journaux de ressources, journal d’activité
...
- Les **métriques de la plateforme Azure** sont disponibles en moins d’une minute dans la base de données de métriques, mais elles prennent 3 minutes supplémentaires pour être exportées vers le point d’ingestion des journaux Azure Monitor.
...
- Les données du **journal d’activité** sont ingérées en 30 secondes lorsque vous utilisez les paramètres de diagnostic recommandés au niveau de l’abonnement pour les envoyer dans les journaux d’Azure Monitor. Toutefois, elles peuvent prendre 10 à 15 minutes si vous utilisez l’intégration héritée à la place.  
...
Reportez-vous à la [documentation de chaque solution](/azure/azure-monitor/insights/solutions) afin de déterminer sa fréquence de collecte.
...
**Généralement moins de 1 minute, mais peut être plus** 
