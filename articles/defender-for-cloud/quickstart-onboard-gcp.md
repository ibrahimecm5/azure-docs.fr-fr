...
> [!IMPORTANT]
> Pendant l’événement Ignite Fall 2021, nous avons annoncé une mise à jour pour la connexion de vos comptes à partir d’autres fournisseurs de cloud. Cette mise à jour utilise la page **Paramètres d’environnement**. Les comptes GCP ne sont pas pris en charge à partir de cette page. Pour connecter un compte GCP à votre abonnement Azure, vous devez utiliser l’expérience des connecteurs cloud classiques, comme décrit ci-dessous.

:::image type="content" source="./media/quickstart-onboard-gcp/gcp-account-in-overview.png" alt-text="Capture d’écran des projets GCP affichés dans le tableau de bord de vue d’ensemble de Microsoft Defender pour le cloud." lightbox="./media/quickstart-onboard-gcp/gcp-account-in-overview.png":::
