---
title: Intégrer une application au réseau virtuel Azure
...
ms.locfileid: "130270778"
---
# <a name="integrate-your-app-with-an-azure-virtual-network"></a>Intégrer votre application à un réseau virtuel Azure

Cet article décrit la fonctionnalité d’intégration au réseau virtuel Azure App Service et comment la configurer avec des applications dans [Azure App Service](./overview.md). Avec les [réseaux virtuels Azure](../virtual-network/virtual-networks-overview.md) (VNets), vous pouvez placer un grand nombre de vos ressources Azure dans un réseau routable non-Internet. La fonctionnalité d’intégration au réseau virtuel permet à vos applications d’accéder à des ressources dans ou via un réseau virtuel. Elle n’autorise pas l’accès privé à vos applications.
...
L’intégration au réseau virtuel régional prend en charge la connexion à un VNet dans la même région et ne nécessite pas de passerelle. L’utilisation de l’intégration au réseau virtuel régional permet à votre application d’accéder aux :
...
Quand vous utilisez l’Intégration au réseau virtuel régional, vous pouvez utiliser les fonctionnalités de réseau Azure suivantes :
...
> * Les conteneurs Windows ne prennent pas en charge le routage des références Key Vault d’App Service ou le tirage d’images conteneur personnalisées via l’intégration au réseau virtuel.
...
Les règles de trafic entrant dans un NSG ne s’appliquent pas à votre application, car l’intégration au réseau virtuel affecte seulement le trafic sortant de votre application. Pour contrôler le trafic entrant vers votre application, utilisez la fonctionnalité Restrictions d’accès.

### <a name="service-endpoints"></a>Points de terminaison de service

L’intégration au réseau virtuel régional vous permet d’accéder aux services Azure qui sont sécurisés avec des points de terminaison de service. Pour accéder à un service sécurisé par point de terminaison de service, vous devez effectuer les opérations suivantes :

* Configurez l’intégration au réseau virtuel régional avec votre application web afin de vous connecter à un sous-réseau spécifique pour l’intégration.
...
Si vous souhaitez effectuer des appels vers des [points de terminaison privés](./networking/private-endpoint.md), vous devez vous assurer que vos recherches DNS seront résolues sur le point de terminaison privé. Vous pouvez appliquer ce comportement de l’une des façons suivantes : 

* Intégrer à des zones privées Azure DNS Si votre réseau virtuel n’a pas de serveur DNS personnalisé, cette intégration est effectuée automatiquement quand les zones sont liées au VNet.
* Gérer le point de terminaison privé dans le serveur DNS utilisé par votre application. Pour gérer la configuration, vous devez connaître l’adresse IP du point de terminaison privé, puis pointer le point de terminaison auquel vous essayez d’accéder vers cette adresse en utilisant un enregistrement A.
...
* La fonctionnalité nécessite un sous-réseau inutilisé avec un bloc IPv4 `/28` au moins dans un VNet Azure Resource Manager.
...
* Vous pouvez avoir seulement une intégration au réseau virtuel régional par plan App Service. Plusieurs applications d’un même plan App Service peuvent utiliser le même réseau virtuel.
...
## <a name="gateway-required-vnet-integration"></a>Intégration au réseau virtuel avec passerelle

L’intégration au réseau virtuel avec passerelle prend en charge la connexion à un VNet d’une autre région ou à un réseau virtuel classique. Intégration au réseau virtuel avec passerelle :
...
Vous ne pouvez pas utiliser l’intégration au réseau virtuel avec passerelle dans les conditions suivantes :
...
Si vous créez la passerelle pour l’utiliser avec l’intégration au réseau virtuel avec passerelle, vous n’avez pas besoin de charger un certificat. La création de la passerelle peut prendre 30 minutes. Vous ne pouvez pas intégrer votre application à votre VNet tant que la passerelle n’est pas créée.

### <a name="how-gateway-required-vnet-integration-works"></a>Fonctionnement de l’intégration au réseau virtuel avec passerelle

L’intégration au réseau virtuel avec passerelle s’appuie sur la technologie VPN point à site. Les VPN de point à site limitent l’accès réseau à la seule machine virtuelle qui héberge l’application. Les applications sont limitées à l’envoi du trafic vers Internet uniquement sur des connexions hybrides ou avec l’intégration au réseau virtuel. Quand votre application est configurée avec le portail pour utiliser l’intégration au réseau virtuel avec passerelle, une négociation complexe est gérée en votre nom pour créer et attribuer des certificats sur la passerelle et du côté de l’application. Au final, les Workers utilisés pour héberger vos applications sont en mesure de se connecter directement à la passerelle de réseau virtuel, dans le réseau virtuel sélectionné.

:::image type="content" source="./media/overview-vnet-integration/vnetint-how-gateway-works.png" alt-text="Fonctionnement de l’intégration au réseau virtuel avec passerelle":::

### <a name="access-on-premises-resources"></a>Accès aux ressources locales

Les applications peuvent accéder aux ressources locales en s’intégrant à des réseaux virtuels qui ont des connexions site à site. Si vous utilisez l’intégration au réseau virtuel avec passerelle, mettez à jour les routes de votre passerelle VPN locale avec vos blocs d’adresses point à site. Lors de la configuration initiale de ce VPN site à site, les scripts utilisés pour le configurer doivent définir les routes correctement. Si vous ajoutez des adresses de point à site après avoir créé votre VPN de site à site, vous devez mettre à jour les routes manuellement. La procédure détaillée dépend de la passerelle et n’est pas décrite ici. Vous ne pouvez pas configurer BGP avec une connexion VPN de site à site.

Aucune configuration supplémentaire n’est nécessaire pour que la fonctionnalité d’intégration au réseau virtuel régional puisse accéder à vos ressources locales via votre réseau virtuel. Vous devez simplement connecter votre réseau virtuel à vos ressources locales à l’aide d’ExpressRoute ou d’un VPN de site à site.

> [!NOTE]
> La fonctionnalité d’intégration au réseau virtuel avec passerelle n’intègre pas d’application à un réseau virtuel doté d’une passerelle ExpressRoute. Même si la passerelle ExpressRoute est configurée en [mode de coexistence](../expressroute/expressroute-howto-coexist-resource-manager.md), l’intégration au réseau virtuel ne fonctionne pas. Pour accéder à des ressources via une connexion ExpressRoute, utilisez la fonctionnalité d’intégration au réseau virtuel régional ou un [environnement App Service](./environment/intro.md), qui s’exécute dans votre VNet.
...
Si vous utilisez le peering avec l’intégration au réseau virtuel avec passerelle, vous devez configurer quelques éléments supplémentaires. Pour configurer le peering afin qu’il fonctionne avec votre application :
...
### <a name="gateway-required-vnet-integration-routing"></a>Routage de l’intégration au réseau virtuel avec passerelle
Les routes définies dans votre réseau virtuel sont utilisées pour diriger le trafic dans votre réseau virtuel à partir de votre application. Pour envoyer du trafic sortant supplémentaire dans le réseau virtuel, ajoutez ces blocs d’adresses ici. Cette fonctionnalité fonctionne seulement avec l’intégration au réseau virtuel avec passerelle. Les tables de route n’affectent pas le trafic de votre application quand vous utilisez l’intégration au réseau virtuel avec passerelle comme elles le font avec l’intégration au réseau virtuel régional.

### <a name="gateway-required-vnet-integration-certificates"></a>Certificats de l’intégration au réseau virtuel avec passerelle
Quand l’intégration au réseau virtuel avec passerelle est activée, un échange de certificats est nécessaire pour garantir la sécurité de la connexion. En plus des certificats, la configuration DNS, les routes et d’autres éléments similaires décrivent le réseau.
...
L’utilisation de la fonctionnalité d’intégration au réseau virtuel régional n’engendre aucuns frais en plus de ceux liés au niveau tarifaire du plan App Service.

Trois types de frais sont liés à l’utilisation de la fonctionnalité d’intégration au réseau virtuel avec passerelle :

* **Frais liés au niveau tarifaire du plan App Service** : Vos applications doivent être dans un plan App Service Standard, Premium, PremiumV2 ou PremiumV3. Pour plus d’informations sur ces frais, consultez [Tarification d’App Service](https://azure.microsoft.com/pricing/details/app-service/).
