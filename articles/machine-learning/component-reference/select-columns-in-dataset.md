---
title: 'Sélectionner des colonnes dans le jeu de données : référence du composant'
titleSuffix: Azure Machine Learning
description: Découvrez comment utiliser le composant Sélectionner des colonnes dans le jeu de données d’Azure Machine Learning pour choisir un sous-ensemble de colonnes à utiliser dans des opérations en aval.
...
ms.locfileid: "131566908"
---
# <a name="select-columns-in-dataset-component"></a>Composant Sélectionner des colonnes dans le jeu de données

Cet article décrit un composant dans le concepteur Azure Machine Learning.

Utilisez ce composant pour choisir un sous-ensemble de colonnes à utiliser dans des opérations en aval. Le composant ne supprime pas physiquement les colonnes de jeu de données source. Au lieu de cela, il crée un sous-ensemble de colonnes, à l’instar d’une *vue* ou *projection* de base de données.

Ce composant est utile lorsque vous devez limiter les colonnes disponibles pour une opération en aval, ou si vous souhaitez réduire la taille du jeu de données en supprimant les colonnes inutiles.
...
Ce composant n’a pas de paramètres. Vous utilisez le sélecteur de colonne pour choisir les colonnes à inclure ou exclure.

### <a name="choose-columns-by-name"></a>Choisir des colonnes par nom

Plusieurs options du composant permettent de choisir des colonnes par nom : 

+ Filtrer et rechercher

    Cliquez sur l’option **PAR NOM**.

    Si vous avez connecté un jeu de données déjà rempli, une liste des colonnes disponibles doit apparaître. Si aucune colonne n’apparaît, vous devrez peut-être exécuter des composants en amont pour afficher la liste des colonnes.
...
+ Saisir ou coller une liste de noms de colonnes séparée par des virgules
    ...
    3. Collez ou saisissez une liste de noms de colonnes validés précédemment séparée par des virgules. Vous ne pouvez pas enregistrer le composant si une colonne a un nom non valide. Veillez donc à vérifier les noms au préalable.
