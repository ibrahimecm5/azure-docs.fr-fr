---
title: 'Allocation de Dirichlet latente : référence du composant'
titleSuffix: Azure Machine Learning
description: Découvrez comment utiliser le composant Allocation de Dirichlet latente pour regrouper du texte non classifié en catégories.
...
ms.locfileid: "131566756"
---
# <a name="latent-dirichlet-allocation-component"></a>composant Allocation de Dirichlet latente

Cet article explique comment utiliser le composant Allocation de Dirichlet latente dans le concepteur Azure Machine Learning pour regrouper du texte non classifié en catégories. 
...
Ce composant prend une colonne de texte et génère les sorties suivantes :
...
Ce composant utilise la bibliothèque scikit-learn. Pour plus d’informations sur Scikit-learn, consultez le [référentiel GitHub](https://github.com/scikit-learn/scikit-learn), qui comprend des tutoriels et une explication de l’algorithme.
...
L’implémentation de ce composant est basée sur la [bibliothèque scikit-learn](https://github.com/scikit-learn/scikit-learn/blob/master/sklearn/decomposition/_lda.py) pour LDA.
...
Ce composant requiert un jeu de données qui contient une colonne de texte, à l’état brut ou prétraité.

1. Ajoutez le composant **Allocation de Dirichlet latente** à votre pipeline.
...
6. Sélectionnez l’option **Normalize** pour convertir les valeurs de sortie en probabilités. 
    ...
    > [!NOTE] 
    > Dans le concepteur Azure Machine Learning, la bibliothèque scikit-learn ne prend plus en charge la sortie non normalisée *doc_topic_distr* depuis la version 0.19. Dans ce composant, le paramètre **Normalize** ne peut être appliqué qu’à la sortie *Feature Topic matrix*. La sortie *Transformed dataset* est toujours normalisée.
...
Ce composant génère également la *transformation LDA* qui applique LDA au jeu de données.
...
Pour réutiliser cette transformation, sélectionnez l’icône **Enregistrer le jeu de données** dans le volet droit du composant Allocation de Dirichlet latente pour conserver le composant sous la catégorie **Jeux de données** dans la liste des composants. Vous pouvez ensuite connecter ce composant au composant [Appliquer la transformation](apply-transformation.md) pour réutiliser cette transformation.
...
Pour obtenir la liste des erreurs propres aux composants, consultez [Exceptions et codes d’erreur pour le concepteur](designer-error-codes.md).
