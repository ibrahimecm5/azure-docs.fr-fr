---
title: 'Clustering k-moyennes : référence du composant'
titleSuffix: Azure Machine Learning
description: Découvrez comment utiliser le composant Clustering k-moyennes dans Azure Machine Learning pour effectuer l’apprentissage de modèles de clustering.
...
ms.locfileid: "131566833"
---
# <a name="component-k-means-clustering"></a>Composant : Clustering k-moyennes

Cet article décrit comment utiliser le composant *Clustering k-moyennes* dans le concepteur Azure Machine Learning pour créer un modèle de clustering k-moyennes non formé. 
...
* Ajoutez ce composant à votre pipeline.
...
Une fois que vous avez configuré les hyperparamètres du composant, vous connectez le modèle non formé au composant [Effectuer l’apprentissage du modèle de clustering](train-clustering-model.md). Étant donné que l’algorithme k-moyennes est une méthode d’apprentissage non supervisée, une colonne d’étiquette est facultative. 
...
-   L’algorithme a terminé l’exécution du nombre spécifié d’itérations.  
 ...
 Une fois que vous avez terminé la phase de formation, vous utilisez le composant [Attribuer des données à des clusters](assign-data-to-clusters.md) pour attribuer de nouveaux cas à l’un des clusters que vous avez trouvés à l’aide de l’algorithme k-moyennes. Vous réalisez une attribution de cluster en calculant la distance entre le nouveau cas et le centroïde de chaque cluster. Chaque nouveau cas est attribué au cluster au centroïde le plus proche.  

## <a name="configure-the-k-means-clustering-component"></a>Configurer le composant de clustering k-moyennes
  
1.  Ajoutez le composant **Clustering k-moyennes** à votre pipeline.  
...
10. Effectuez l’apprentissage du modèle.  
    ...
    -   Si vous définissez le **mode Create trainer** (Créer formateur) sur **Paramètre unique**, ajoutez un jeu de données balisé et formez le modèle à l’aide du composant [Effectuer l’apprentissage du modèle de clustering](train-clustering-model.md).  
...
Si vous avez utilisé le composant [Effectuer l’apprentissage du modèle de clustering](train-clustering-model.md) :

1. Sélectionnez le composant **Effectuer l’apprentissage du modèle de clustering** et ouvrez le volet droit.
...
1. Cliquez avec le bouton droit sur le composant **Effectuer l’apprentissage du modèle de clustering**.
