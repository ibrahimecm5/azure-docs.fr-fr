...
description: Découvrez comment utiliser le composant SMOTE dans Azure Machine Learning pour augmenter le nombre d’exemples à faible incidence dans un jeu de données à l’aide d’un suréchantillonnage.
...
1.  Ajoutez le composant SMOTE à votre pipeline. Vous trouverez ce composant sous **Composants de transformation de données**, dans la catégorie **Manipulation**.

2. Connectez le jeu de données que vous souhaitez enrichir. Si vous souhaitez spécifier l’espace de caractéristiques qui servira à générer les nouveaux cas, soit en utilisant seulement certaines colonnes, soit en en excluant d’autres, utilisez le composant [Sélectionner des colonnes dans le jeu de données](select-columns-in-dataset.md). Vous pouvez ensuite isoler les colonnes à utiliser avant de vous servir de SMOTE.
...
4.  Le composant SMOTE identifie automatiquement la classe minoritaire dans la colonne d’étiquette, puis obtient tous les exemples de cette classe. Toutes les colonnes ne peuvent pas avoir de valeurs NaN.
...
5.  Dans l’option **Pourcentage SMOTE**, entrez un nombre entier indiquant le pourcentage cible de cas minoritaires dans le jeu de données de sortie. Par exemple :  
    ...
    - Vous entrez **0**. Le composant SMOTE retourne exactement le même jeu de données que celui que vous avez fourni en entrée. Il n’ajoute aucun nouveau cas minoritaire. Dans ce jeu de données, la proportion des classes n’a pas changé.  
  
    - Vous entrez **100**. Le composant SMOTE génère de nouveaux cas minoritaires. Il ajoute le même nombre de cas minoritaires que dans le jeu de données d’origine. Comme SMOTE n’augmente pas le nombre de cas majoritaires, la proportion des cas de chaque classe a changé.  
  
    - Vous entrez **200**. Le composant double le pourcentage de cas minoritaires par rapport au jeu de données d’origine. Cela ne signifie *pas* obtenir deux fois plus de cas minoritaires qu’avant. Au lieu de cela, la taille du jeu de données est augmentée de telle sorte que le nombre de cas majoritaires reste le même. Le nombre de cas minoritaires est augmenté jusqu’à ce qu’il corresponde à la valeur de pourcentage souhaitée.  
...
7. Entrez une valeur dans la zone **Random seed** (Valeur de départ aléatoire) si vous souhaitez obtenir les mêmes résultats à chaque exécution du même pipeline avec les mêmes données. Sinon, le composant générera une valeur de départ aléatoire à partir des valeurs d’horloge du processeur au moment du déploiement du pipeline. La génération d’une valeur de départ aléatoire peut provoquer de légères différences entre les résultats au fil des exécutions.

8. Envoyez le pipeline.  
  
   La sortie du composant est un jeu de données contenant les lignes d’origine ainsi que diverses lignes ajoutées avec des cas minoritaires.  

## <a name="technical-notes"></a>Notes techniques

+ Lorsque vous publiez un modèle qui utilise le composant **SMOTE**, supprimez **SMOTE** du pipeline prédictif avant qu’il soit publié en tant que service web . En effet, SMOTE est destiné à l’amélioration d’un modèle pendant l’entraînement, et non au scoring. Une erreur pourrait se produire si le pipeline prédictif publié contenait le composant SMOTE.
