---
title: 'Régression linéaire : référence du composant'
titleSuffix: Azure Machine Learning
description: Découvrez comment utiliser le composant Régression linéaire dans Azure Machine Learning pour créer un modèle de régression linéaire à utiliser dans un pipeline.
...
ms.locfileid: "131566829"
---
# <a name="linear-regression-component"></a>Composant Régression linéaire
Cet article décrit un composant dans le concepteur Azure Machine Learning.

Utilisez ce composant pour créer un modèle de régression linéaire à utiliser dans un pipeline.  Une régression linéaire tente d’établir une relation linéaire entre une ou plusieurs variables indépendantes et un résultat numérique ou une variable dépendante. 

Ce composant vous permet de définir une méthode de régression linéaire, puis d’effectuer l’apprentissage d’un modèle à l’aide d’un jeu de données étiqueté. Le modèle ainsi formé permet ensuite d’effectuer des prédictions.
...
+ La régression classique implique une variable indépendante et une variable dépendante. C’est ce qu’on appelle une *régression simple*.  Ce composant prend en charge la régression simple.
...
+ Une *régression linéaire multiple* implique au moins deux variables indépendantes qui contribuent à déterminer une seule variable dépendante. Lorsque plusieurs entrées sont utilisées pour prédire un résultat numérique unique, on parle également de *régression linéaire multivariée ou multidimensionnelle*.
    ...
    Le composant **Régression linéaire** peut résoudre ces problèmes, à l’instar de la plupart des autres composants de régression.
...
Ce composant prend en charge deux méthodes d’ajustement d’un modèle de régression, avec différentes options :
...
1. Ajoutez le composant **Modèle de régression linéaire** à votre pipeline dans le concepteur.

    Ce composant figure dans la catégorie **Machine Learning**. Développez **Initialiser un modèle**, développez **Régression**, puis faites glisser le composant **Modèle de régression linéaire** vers votre pipeline.
...
7. Ajoutez le composant [Entraîner le modèle](./train-model.md) à votre pipeline, puis connectez un jeu de données étiqueté.
...
+ Pour effectuer des prédictions, connectez le modèle formé au composant [Score Model](./score-model.md) (Noter le modèle), ainsi qu’un jeu de données de nouvelles valeurs. 
...
6. **Normaliser les fonctionnalités** : si vous avez déjà normalisé les données numériques utilisées pour l’apprentissage du modèle, vous pouvez désactiver cette option. Par défaut, le composant normalise toutes les entrées numériques en les ramenant à une plage comprise entre 0 et 1.
...
+ Pour effectuer des prédictions, connectez le modèle formé au composant [Score Model](./score-model.md) (Noter le modèle), ainsi que de nouvelles données d’entrée.
