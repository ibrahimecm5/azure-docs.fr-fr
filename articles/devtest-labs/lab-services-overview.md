...
- **Azure DevTest Labs** : ce service vous permet de configurer rapidement un environnement pour votre équipe, par exemple un environnement de développement ou un environnement de test dans le cloud. Le propriétaire d’un laboratoire crée un laboratoire, configure Windows ou des machines virtuelles Linux, installe les logiciels et outils nécessaires, puis les rend disponibles aux utilisateurs du laboratoire. Les utilisateurs du lab se connectent à des machines virtuelles dans le lab, puis s’en servent pour leurs tâches quotidiennes ou leurs projets à court terme. Quand les utilisateurs commencent à utiliser les ressources du labo, un administrateur de labo peut analyser les coûts et l’utilisation entre plusieurs labos et définir des stratégies globales afin d’optimiser les coûts de l’équipe ou de l’organisation.
- **Azure Lab Services** : ce service vous permet de créer des types de labs managés. Les labos de salle de classe sont le seul type de labo managé pris en charge par Azure Lab Services. Le service lui-même gère toute l’infrastructure d’un type de labo managé, de l’augmentation de la capacité des machines virtuelles à la gestion des erreurs, en passant par la mise à l’échelle de cette infrastructure. Une fois qu’un administrateur informatique a créé un compte Lab dans Azure Lab Services, un formateur peut rapidement configurer un labo de classe. L’instructeur peut spécifier le nombre et le type de machines virtuelles nécessaires pour effectuer des exercices dans la classe, et ajouter des utilisateurs à la classe. Une fois qu’un utilisateur s’est inscrit dans la classe, il peut accéder à la machine virtuelle pour effectuer les exercices de la classe.  

## <a name="key-capabilities"></a>Fonctionnalités clés

Azure DevTest Labs et Azure Lab Services prennent en charge les fonctionnalités clés suivantes :

- **Configuration rapide et flexible d’un laboratoire**. Grâce à Azure Lab Services, les propriétaires de laboratoire peuvent rapidement configurer un laboratoire selon leurs besoins. Le service permet de prendre en charge tous les travaux d’infrastructure Azure pour les types de labo managé. Ou vous pouvez permettre aux propriétaires de labo de gérer et de personnaliser l’infrastructure de façon autonome dans l’abonnement du propriétaire du labo. Le service intègre une mise à l’échelle et une résilience de l’infrastructure pour les laboratoires que le service gère pour vous.
- **Expérience simplifiée pour les utilisateurs du laboratoire**. Dans un labo managé, comme un labo de salle de classe, les utilisateurs peuvent s’inscrire avec un code d’inscription et accéder ainsi à tout moment aux ressources du labo. Le propriétaire d’un labo peut autoriser les utilisateurs à créer et à accéder aux machines virtuelles, à gérer et à réutiliser des disques de données, et à configurer des clés secrètes réutilisables.  
- **Optimisation et analyse des coûts**. Un propriétaire du laboratoire peut définir des planifications de laboratoire pour arrêter et démarrer automatiquement des machines virtuelles. Le propriétaire du labo peut définir une planification pour spécifier les créneaux horaires pendant lesquels les machines virtuelles du labo sont accessibles aux utilisateurs. Le propriétaire peut définir des stratégies d’utilisation par utilisateur ou par labo pour optimiser les coûts et analyser les tendances d’utilisation et d’activité dans un labo. Pour les types de labos managés comme les labos de salle de classe, un sous-ensemble plus petit d’options d’optimisation et d’analyse des coûts est disponible.
- **Sécurité intégrée**. Le propriétaire du laboratoire peut configurer des réseaux virtuels privés et un sous-réseau pour un laboratoire, et activer une adresse IP publique partagée. Les utilisateurs du laboratoire peuvent accéder en toute sécurité aux ressources à l’aide du réseau virtuel configuré avec ExpressRoute ou d’un VPN de site à site. (Uniquement disponible dans DevTest Labs.)
- **Intégration dans vos flux de travail et outils**. Azure Lab Services vous permet d’intégrer les laboratoires dans le site Web et les systèmes de gestion de votre organisation. Vous pouvez automatiquement mettre en service les environnements à l’aide de vos outils de déploiement/intégration (CI/CD) en continu. (Uniquement disponible dans DevTest Labs.)
...
- Créer un laboratoire de salle de classe managé. Il vous suffit d’indiquer au service exactement ce dont vous avez besoin. Le service crée et gère l’infrastructure du labo. Vous pouvez ainsi vous concentrer sur votre classe, sans vous soucier des aspects techniques d’un labo.
...
- Les administrateurs peuvent contrôler les coûts en s’assurant que les développeurs ne peuvent pas obtenir plus de machines virtuelles que nécessaire.
- Les administrateurs peuvent s’assurer que les machines virtuelles sont arrêtées quand elles ne sont pas utilisées.
...
- Les administrateurs peuvent contrôler les coûts en s’assurant que les testeurs ne peuvent pas obtenir plus de machines virtuelles que nécessaire.
- Les administrateurs peuvent s’assurer que les machines virtuelles sont arrêtées quand elles ne sont pas utilisées.
...
- Configurer un pool de machines virtuelles offrant des performances élevées en calcul pour effectuer des recherches nécessitant beaucoup de ressources système ou de graphismes. Exécuter les machines virtuelles suivant vos besoins, et les nettoyer une fois que vous avez terminé. 
- Déplacer vers le cloud l’ordinateur physique qui servait de laboratoire à votre établissement. Redimensionner automatiquement et exclusivement le nombre de machines virtuelles à la limite maximale d’utilisation et au seuil de coûts définis sur le laboratoire.  
...
- Créer à plusieurs reprises un laboratoire de machines de test dans le cadre d’un déploiement. Effectuer les derniers tests et nettoyer les machines de test une fois que vous avez terminé. 
